...
pacman::p_load(here, labelled, hablar, MASS, tidyverse, VIM, Hmisc, psych, margins, pscl, xtable)
source(here("02_Code/util.R"))



# Functions ---------------------------------------------------------------
# Function to extract relevant info from model
clean_results <- function(model, ndigits=3){
  res <- summary(model)$coefficients %>% 
    ...
    mutate(odds_p = case_when(
      p_value < 0.01 ~ paste0(round(odds, ndigits), "***"),
      p_value < 0.05 & p_value >= 0.01 ~ paste0(round(odds, ndigits), "**"),
      p_value < 0.1 & p_value >= 0.05 ~ paste0(round(odds, ndigits), "*"),
      p_value >= 0.1 ~ paste0(round(odds, ndigits))
    )) %>% 
    select(variable, odds_p) %>% 
    bind_rows(tibble(variable = "N", odds_p = as.character(nobs(model)))) %>% 
    bind_rows(tibble(variable = "BIC", odds_p = as.character(round(BIC(model), ndigits)))) %>% 
    bind_rows(tibble(variable = "Pseudo_R2", odds_p = as.character(round(pR2(model)['r2ML'], ndigits)))) 
  
  return(res)
}

clean_results_logit <- function(model, ndigits=3){
  ...
  res <- margins_summary(model) %>% 
    ...
    mutate(ame_p = case_when(
      p < 0.01 ~ paste0(round(AME, ndigits), "***"),
      p < 0.05 & p >= 0.01 ~ paste0(round(AME, ndigits), "**"),
      p < 0.1 & p >= 0.05 ~ paste0(round(AME, ndigits), "*"),
      p >= 0.1 ~ paste0(round(AME, ndigits))
    )) %>% 
    select(factor, ame_p) %>% 
    arrange(helper) %>% 
    bind_rows(tibble(factor = "N", ame_p = as.character(nobs(model)))) %>% 
    bind_rows(tibble(factor = "BIC", ame_p = as.character(round(BIC(model), ndigits)))) %>% 
    bind_rows(tibble(factor = "Pseudo_R2", ame_p = as.character(round(pR2(model)['r2ML'], ndigits)))) 
...
df_model <- df_startup %>% 
  ...
  ungroup() %>% 
  select(gpkey, age, produkt_kat, branche11, team, anzteam, foe, bes, fue, fuep, fuek_ums, fuep_bes, fuex, fuek, patj, anzpat, umsj, ums, gewj, gew, contains("umwelt_wirk"), contains("umwelt_inno"), cleantech_fields, url, text_en)
...
df_model <- kNN(df_model, 
                metric = "mahalanobis", 
                variable = c("team", "anzteam", "fue", "fuep", "umsj", "foe", "bes", "fuep_bes", "fuek_ums"), 
...
desc_list[[1]] <- apply(df_model %>% select(cleantech, tech_prox, foe, bes, fuek_ums, fuep_bes, fue, fuep, fuex, fuek, patj, anzpat, umsj, ums, gewj, gew, age, team, anzteam, cleantech_fields, contains("umwelt")), MARGIN = 2, function(x) Hmisc::describe(x))
desc_list[[2]] <- sapply(df_model %>% select(cleantech, tech_prox, foe, bes, fuek_ums, fuep_bes, fue, fuep, fuex, fuek, umsj, ums, gewj, gew, age, team, anzteam, cleantech_fields, 
...
model_equations <- lapply(cleantech_fields, function(tech) paste("umwelt_wirk_cat ~", tech, "+ branche11 + produkt_kat")) %>% as_vector()
...
for (i in seq_along(model_equations)){
  ...
  coef_results <- clean_results(polr_mod, variables = c(cleantech_fields, str_detect()))
  result_table <- result_table %>% bind_rows(tibble(model = model_equation, coef_results))
...
for (i in seq_along(model_equations)){
  ...
  polr_mod <- polr(formula = as.formula(model_equation), data = df_model3, na.action = na.omit)
  model_list[[i]] <- polr_mod
  
  coef_results <- clean_results(polr_mod, variables = c(cleantech_fields, "fue", "anzteam", "age"))
  result_table <- result_table %>% bind_rows(tibble(model = model_equation, coef_results))
...
model_equations <- c(  
  ...
  #"umwelt_wirk_cat ~ tech_prox + branche11 + log(bes) + age + scale(fuek_ums)",
  #"umwelt_wirk_cat ~ tech_prox + branche11 + log(bes) + age + fuep_bes",
  "umwelt_wirk_cat ~ tech_prox + branche11 + log(bes) + age + log(fuek + 1)",
  "umwelt_wirk_cat ~ tech_prox + branche11 + log(bes) + age + fue + foe",
  "umwelt_wirk_cat ~ tech_prox + branche11 + log(bes) + age + fue + foe + umsj",
  "umwelt_wirk_cat ~ tech_prox + branche11 + log(bes) + age + fue + foe + log(ums + 1)",
  "umwelt_wirk_cat ~ tech_prox + branche11 + log(bes) + age + fue + foe + umsj + gewj",
  "umwelt_wirk_cat ~ tech_prox + branche11 + log(bes) + age + fue + foe + umsj + gewj + produkt_kat"
)
...
model_equations <- c(  
  ...
  "umwelt_inno_cat ~ tech_prox + branche11 + log(bes) + age + fue + foe + umsj + gewj + produkt_kat"
)

...
df_web = pd.read_excel(here(r".\01_Data\02_Firms\03_StartupPanel\df_no_desc_but_url.xlsx"))

df_web

df_temp = df_web.loc[df_web.text!='-'].head(3)

df_temp['text_en'] = df_temp.text.progress_apply(lambda x: GoogleTranslator().translate(x))

df_temp[['text', 'text_en']].style

df_temp = df_web.loc[df_web.text!='-']
...
df_web = df_web.merge(df_temp[['gpkey', 'text_en']], on='gpkey', how='left')

df_web

df_startup = pd.read_csv(here('01_Data/02_Firms/df_gp_en.txt'), sep='\t')

# Create indicator variable showing which texts come from websites and replace missing text descriptions with webtexts
df_startup.loc[df_startup.gpkey.isin(df_web.gpkey.values), 'web'] = 1
df_startup = df_startup.merge(df_web[['gpkey', 'text', 'text_en']].rename(columns={'text':'text_web', 'text_en':'text_web_en'}), on='gpkey', how='left')
df_startup.loc[df_startup.text.isnull() & df_startup.text_web.notnull(), 'text'] =  df_startup.loc[df_startup.text.isnull() & df_startup.text_web.notnull()].text_web
df_startup.loc[df_startup.text_en.isnull() & df_startup.text_web_en.notnull(), 'text_en'] =  df_startup.loc[df_startup.text_en.isnull() & df_startup.text_web_en.notnull()].text_web_en
df_startup.drop(columns=['text_web', 'text_web_en'], inplace=True)
...
here('01_Data/02_Firms/df_gp_en_prox.txt')
